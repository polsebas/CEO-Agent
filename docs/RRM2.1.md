...
| 3 | `replay_confidence=1.0` en cierre (baseline en misma TX, sin `validate_frozen_replay` previo) | `core/orchestrator.py` |
...
Tests con bridge Prometheus (`test_prometheus_bridge_*`, `test_metrics_endpoint_*`) habilitan OTel localmente vía `monkeypatch`.
